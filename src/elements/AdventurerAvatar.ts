...
export class ThrowState extends StaggerableState {
  ...
  onEnterState(previousState: State | undefined): void {
    ...
    GameplayScene.instance.dispatcher.queueDelayedFunction(this.stateMachine,()=>{this.enableFragile()}, 0.1);
    this.timeInThrow = 0;
  }

  enableFragile() {
    console.log("enable fragile");
    if (this.liftedItem && this.wasFragile) {
      let fragileElement = this.liftedItem.getElement(Fragile);
      if (fragileElement) {
        fragileElement.enabled = true;
      }
